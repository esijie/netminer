...
namespace NetMiner.Core.Proxy
{
    public class cVerifyProxy
    {
        ...
        //启动爬虫
        public bool Start(List<eProxy> Proxys)
        {
        ...
        //验证代理IP
        private bool VerifyProxy(eProxy p)
        {
            try
            {
                cProxyControl pControl = new cProxyControl(this.m_workPath, p.ProxyServer, int.Parse(p.ProxyPort), p.User, p.Password);

                Uri uri = new Uri(this.VerifyUrl);
                NetMiner.Net.Native.HttpUnity hu = new Net.Native.HttpUnity(this.m_workPath);

                NetMiner.Net.Common.eRequest request = new Net.Common.eRequest();
                request.Cookie = null;
                request.AllAllowAutoRedirect = true;

                WebProxy wp = new WebProxy(p.ProxyServer, int.Parse(p.ProxyPort));
                wp.BypassProxyOnLocal = true;
                if (p.User != "")
                {
                    wp.Credentials = new NetworkCredential(p.User, p.Password);
                }
                NetMiner.Net.Common.eResponse response= hu.RequestUri(uri, request, cGlobalParas.RequestMethod.Get, wp);
        ...
        bool EnqueueUri(eProxy uri, bool bCheckRepetition)
        {
