    ...
    public class cPlans
    {
        ...
        public void NewIndexFile()
        {
            ...
            string strXml = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>" +
                       "<Plans>" +
                       "</Plans>";
            xmlConfig.NewXmlFile(m_workPath + "tasks\\plan\\plan.xml", strXml);
        ...
        {
            return File.Exists(m_workPath + "tasks\\plan\\plan.xml");
        }
        ...
        {
            cXmlIO xmlConfig = new cXmlIO(m_workPath + "tasks\\plan\\plan.xml");
            ...
            for (int i = 0; i < d.Count; i++)
            {
                ...
                p.PlanState =(cGlobalParas.PlanState) int.Parse (d[i].Row["PlanState"].ToString());
                p.IsOverRun = d[i].Row["IsOverRun"].ToString() == "True" ? true : false;
                p.IsDisabled = d[i].Row["IsDisabled"].ToString() == "True" ? true : false;
                p.DisabledType =(cGlobalParas.PlanDisabledType) int.Parse ( d[i].Row["DisabledType"].ToString());
                ...
                p.IsTimeRange = d[i].Row["IsTimeRange"].ToString() == "True" ? true : false;
                p.StartTime = d[i].Row["StartTime"].ToString();
                p.EndTime = d[i].Row["EndTime"].ToString();
        ...
        public cPlan GetSinglePlan(Int64 PlanID)
        {
            ...
            p.PlanState = (cGlobalParas.PlanState) int.Parse(d[0].Row["PlanState"].ToString());
            p.IsOverRun = d[0].Row["IsOverRun"].ToString() == "True" ? true : false;
            p.IsDisabled = d[0].Row["IsDisabled"].ToString() == "True" ? true : false;
            p.DisabledType = (cGlobalParas.PlanDisabledType) int.Parse(d[0].Row["DisabledType"].ToString());
            ...
            p.IsTimeRange = d[0].Row["IsTimeRange"].ToString() == "True" ? true : false;
            p.StartTime = d[0].Row["StartTime"].ToString();
            p.EndTime = d[0].Row["EndTime"].ToString();

            cTaskPlan tp;
            for (int i = 0; i < t.Count; i++)
            {
                tp = new cTaskPlan();
                tp.RunTaskType =(cGlobalParas.RunTaskType)int.Parse ( t[i].Row["RunTaskType"].ToString());
        ...
        public void InsertPlan(cPlan NewPlan)
        {
            ...
            if (d != null)
            {
                for (int i = 0; i < d.Count; i++)
                {
                    ...
                    {
                        xmlConfig = null;
                        throw new NetMinerException("已经存在此计划，请修改计划名称，计划名称不能重复！");
                    }
                }
            }
        ...
        public bool RenamePlanName(string OldPlanName, string NewPlanName)
        {
            ...
            //判断计划是否重名，如果重名则需要进行名称修改
            cXmlIO xmlPlan = new cXmlIO(m_workPath  + "tasks\\plan\\plan.xml");
            ...
            if (d != null)
            {
                for (int i = 0; i < d.Count; i++)
                {
                    ...
                    {
                        xmlPlan = null;
                        throw new NetMinerException("已经存在此计划，重命名失败");
                        //return false;
                    }
                }
            }
