    ...
    public class cUrlAnalyze
    {
        ...
        public List<string> PUrlRule(List<string> pUrl,int Level, List<cNavigRule> nRules,cGlobalParas.WebCode webCode, string cookie)
        {
            ...
            for (i = 0; i < pUrl.Count; i++)
            {
                ...
                tmpUrls = GetUrlsByRule(pUrl[i].ToString(),HtmlStartPos,HtmlEndPos , UrlRule,webCode,cookie);
        ...
        //根据导航规则，获取网页地址，是一个集合
        public List<string> GetUrlsByRule(string Url,string strStartPos,string strEndPos, string UrlRule,cGlobalParas.WebCode webCode, string cookie)
        {
            string Url1;
            List<string> Urls=new List<string> ();
            ...
            string UrlSource = gW.GetHtml(Url1, webCode, cookie, strStartPos , strEndPos, true, false);
            gW = null;
            ...
            if (UrlRule.StartsWith("<Start>") || UrlRule.StartsWith("<Prefix>") || UrlRule.StartsWith("<Regex>"))
            {
                //复杂的导航规则

                //判断导航规则中是否存在导航结果附加前缀字符串和尾部增加字符串的操作
                ...
                if (Regex.IsMatch(UrlRule, "<Start>"))
                {
                    //Rule = @"(?<=[href=|src=|open(][\W])";

                    //处理前缀
                    charSetMatch = Regex.Match(UrlRule, "(?<=<Start>).*?(?=</Start>)", RegexOptions.IgnoreCase | RegexOptions.Multiline);
                    string strPre = charSetMatch.Groups[0].ToString();
                    Rule += strPre;

                    //处理中间内容
                    string cma = @"(?<=<Common>)\S+?(?=</Common>)";
                    ...
                    {
                        charSetMatch = Regex.Match(UrlRule, "(?<=<End>).*?(?=</End>)", RegexOptions.IgnoreCase | RegexOptions.Multiline);
                        Rule += @"(\S*)" + charSetMatch.Groups[0].ToString();
                    }
                    else
                    {
                        Rule += @"(\S[^'"">]*)(?=[\s'""])";
            ...
            foreach (Match ma in aa)
            {
                ...
                string tUrl = ma.Value.ToString();
                tUrl=Regex.Replace(tUrl, "\\s", "%20");

                d1.Rows.Add(strPrefix + tUrl + strSuffix);
            }
        ...
        private List<string> getListUrl(string dicPre)//,bool IsUrlEncode,string UrlEncode
        {
            ...
            switch (dicPre.Substring(0, dicPre.IndexOf(":")))
            {
                ...
                case "ShortDate":
                    ...
                    startYear = int.Parse(aa[0].Groups[0].Value.ToString());
                    startMonth = int.Parse(aa[1].Groups[0].Value.ToString());
                    startDay = int.Parse(aa[2].Groups[0].Value.ToString());

                    endYear = int.Parse(aa[3].Groups[0].Value.ToString());
                    endMonth = int.Parse(aa[4].Groups[0].Value.ToString());
                    endDay = int.Parse(aa[5].Groups[0].Value.ToString());
                    ...
                    for (i = startYear; i <= endYear; i++)
                    {
                        for (int m = startMonth; m <= endMonth; m++)
                        {
                            if (m == endMonth)
                            {
                                ...
                                {
                                    list_Para.Add(i.ToString() + "-" + m.ToString() + "-" + n.ToString());
                                }
                            }
                            else
                            {
                                DateTime d1=DateTime.Parse (i.ToString() + "-" + m.ToString() + "-1");
                                DateTime d2=DateTime.Parse (i.ToString() + "-" + ((int)(m+1)).ToString () + "-1");
                                while (d1<d2)
                                {
                                    d1=d1.AddDays (1);
                                    list_Para.Add(d1.ToString("yyyy-M-d"));
                                }
                            }
                        }
                    }
                    ...
                    for (i = startYear; i <= endYear; i++)
                    {
                        for (int m = startMonth; m <= endMonth; m++)
                        {
                            if (m == endMonth)
                            {
                                for (int n = startDay; n <= endDay; n++)
                                {
                                    DateTime d3 = DateTime.Parse(i.ToString() + "-" + m.ToString() + "-" + n.ToString());
                                    list_Para.Add(d3.ToString("yyyy-MM-dd"));
                                }
                            }
                            else
                            {
                                ...
                                {
                                    d1 = d1.AddDays(1);
                                    list_Para.Add(d1.ToString("yyyy-MM-dd"));
                                }
                            }
                        }
                    }
