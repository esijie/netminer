...
namespace SoukeyNetget.Task
{
    ...
    public class cTask
    {
        ...
        ////递增采集判断条件：
        //private string m_IncrByCondition;
        //public string IncrByCondition
        //{
        ...
        public void Save(string TaskPath)
        {
            ...
            tXml = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>" +
                ...
                "<LoginUrl>" + cTool.ReplaceTrans(this.LoginUrl) + "</LoginUrl>" +
            ...
            tXml += "<Advance>" +
                ...
                "<IsPublishHeader>" + this.IsPublishHeader + "</IsPublishHeader>" +
                "</Advance>";
            ...
            {
                tXml += "<Header>";
                tXml += "<Label>" + this.m_Headers[i].Label + "</Label>";
                tXml += "<LabelValue>" + this.m_Headers[i].LabelValue + "</LabelValue>";
                tXml += "</Header>";
            }
                ...
                for (i = 0; i < this.WebpageLink.Count; i++)
                {
                    ...
                    if (this.WebpageLink[i].IsNavigation ==true)
                    {
                        ...
                        for (int j = 0; j < this.WebpageLink[i].NavigRules.Count; j++)
                        {
                            ...
                            tXml += "<NaviStartPos>" + cTool.ReplaceTrans(this.WebpageLink[i].NavigRules[j].NaviStartPos) + "</NaviStartPos>";
                            tXml += "<NaviEndPos>" + cTool.ReplaceTrans(this.WebpageLink[i].NavigRules[j].NaviEndPos) + "</NaviEndPos>";
                            tXml += "<NagRule>" + cTool.ReplaceTrans(this.WebpageLink[i].NavigRules[j].NavigRule) + "</NagRule>";
                            tXml += "<IsNextPage>" + this.WebpageLink[i].NavigRules[j].IsNaviNextPage + "</IsNextPage>";
                            tXml += "<NextPageRule>" + cTool.ReplaceTrans(this.WebpageLink[i].NavigRules[j].NaviNextPage) + "</NextPageRule>";
                            tXml += "<IsGather>" + this.WebpageLink[i].NavigRules[j].IsGather + "</IsGather>";
                            tXml += "<GatherStartPos>" + cTool.ReplaceTrans(this.WebpageLink[i].NavigRules[j].GatherStartPos) + "</GatherStartPos>";
                            tXml += "<GatherEndPos>" + cTool.ReplaceTrans(this.WebpageLink[i].NavigRules[j].GatherEndPos) + "</GatherEndPos>";
                            tXml += "</NavigationRule>";
                        }
                        tXml +="</NavigationRules>";
                    }
            ...
            if (this.WebpageCutFlag != null)
            {
                for (i = 0; i < this.WebpageCutFlag.Count; i++)
                {
                    ...
                    tXml += "<IsMergeData>" + this.WebpageCutFlag[i].IsMergeData + "</IsMergeData>";
                    tXml += "<NavLevel>" + this.WebpageCutFlag[i].NavLevel + "</NavLevel>";
                    //插入数据输出规则
                    ...
                    {
                        tXml += "<ExportRule>";
                        tXml += "<ExortField>" + this.WebpageCutFlag[i].ExportRules[m].Field + "</ExortField>";
                        tXml += "<ExortRuleType>" + this.WebpageCutFlag[i].ExportRules[m].FieldRuleType + "</ExortRuleType>";
                        tXml += "<ExortRuleCondition>" + cTool.ReplaceTrans(this.WebpageCutFlag[i].ExportRules[m].FieldRule) + "</ExortRuleCondition>";
                        tXml += "</ExportRule>";
        ...
        private void LoadTaskInfo(string FileName)
        {
            ...
            this.GIntervalTime =int.Parse ( xmlConfig.GetNodeValue("Task/Advance/GatherIntervalTime"));
            this.IsCustomHeader = (xmlConfig.GetNodeValue("Task/Advance/IsCustomHeader") == "True" ? true : false);
            this.IsPublishHeader = (xmlConfig.GetNodeValue("Task/Advance/IsPublishHeader") == "True" ? true : false);
