...
namespace SoukeyNetget.Gather
{
    ...
    public class cGatherTask
    {
        ...
        public void Save()
        {
            ...
            //if (m_TaskData.TaskType == cGlobalParas.TaskType.Incremental)
            //{
            //    ///增量采集任务还需更新采增量采集任务的位置信息
            //    ///首先判断是从前之后，还是从后之前
            //    ///如果是从前递增，则不改变任务的起始网址，如果是从后递增，则改变起始网址为当前正在采集的网址
            //    //记录采集的内容和Url
            //    cIncrTasks its = new cIncrTasks();
            //    its.DelTask(m_TaskData.TaskID);

            //    cIncrTask it = new cIncrTask();
            //    it.TaskID = m_TaskData.TaskID;
            //    it.TaskName = m_TaskData.TaskName;
            //    it.TaskType = m_TaskData.TaskType;
            //    it.IncrCondition = m_TaskData.IncrByCondition;
            ...
            //    {
            //        iUrl = new cIncrUrls();
            //        iUrl.Url = m_TaskData.Weblink[i].Weblink;
            //        iUrl.CurUrl = m_TaskData.Weblink[i].NextPageUrl;
            //        it.Urls.Add(iUrl);
            //    }
        ...
        private string GetSaveTaskXml()
        {
            ...
            for (int i = 0; i < m_TaskData.Weblink.Count; i++)
            {
                ...
                tXml += "<NextPageUrl>" + cTool.ReplaceTrans(m_TaskData.Weblink[i].NextPageUrl) + "</NextPageUrl>";
                ...
                if (m_TaskData.Weblink[i].IsNavigation == true)
                {
                    ...
                    for (int j = 0; j < m_TaskData.Weblink[i].NavigRules.Count; j++)
                    {
                        ...
                        tXml += "<NaviStartPos>" + cTool.ReplaceTrans(m_TaskData.Weblink[i].NavigRules[j].NaviStartPos) + "</NaviStartPos>";
                        tXml += "<NaviEndPos>" + cTool.ReplaceTrans(m_TaskData.Weblink[i].NavigRules[j].NaviEndPos) + "</NaviEndPos>";
                        ...
                        tXml += "<NextPageRule>" + cTool.ReplaceTrans(m_TaskData.Weblink[i].NavigRules[j].NaviNextPage) + "</NextPageRule>";
                        tXml += "<IsGather>" + m_TaskData.Weblink[i].NavigRules[j].IsGather + "</IsGather>";
                        tXml += "<GatherStartPos>" + cTool.ReplaceTrans(m_TaskData.Weblink[i].NavigRules[j].GatherStartPos) + "</GatherStartPos>";
                        tXml += "<GatherEndPos>" + cTool.ReplaceTrans(m_TaskData.Weblink[i].NavigRules[j].GatherEndPos) + "</GatherEndPos>";
        ...
        private void SplitTask()
        {
            ...
            m_TaskData.IsDelTempData = t.IsDelTempData;
            m_TaskData.IsSaveSingleFile = t.IsSaveSingleFile;
            m_TaskData.TempDataFile =t.TempDataFile ;
            m_TaskData.IsExportGUrl = t.IsExportGUrl;
            m_TaskData.IsExportGDateTime = t.IsExportGDateTime;
            m_TaskData.IsDataProcess = t.IsDataProcess;

            m_TaskData.GIntervalTime = t.GIntervalTime;

            m_TaskData.IsCustomHeader = t.IsCustomHeader;
            m_TaskData.IsPublishHeader = t.IsPublishHeader;

            if (t.IsCustomHeader == true)
                m_TaskData.Headers = t.Headers;
            ...
            {
                //如果是直接入库，则必须是发布到数据库，其他方式均不支持
                cGlobalParas.PublishType pType = (cGlobalParas.PublishType)(int.Parse (t.ExportType));
                if (pType == cGlobalParas.PublishType.PublishAccess || pType == cGlobalParas.PublishType.PublishMSSql || pType == cGlobalParas.PublishType.PublishMySql)
                {
                    m_TaskData.PublishType = pType;
                    m_TaskData.DataSource = t.DataSource;
                    m_TaskData.TableName = t.DataTableName;
                    m_TaskData.InsertSql = t.InsertSql;
                }
            }
            ...
            if (m_TaskData.UrlCount > m_TaskData.ThreadCount && m_TaskData.ThreadCount > 1)
            {
                ...
                for (int i = 1; i <= m_TaskData.ThreadCount; i++)
                {
                    ...
                    dtc.IsExportGUrl = m_TaskData.IsExportGUrl;
                    dtc.IsExportGDateTime = m_TaskData.IsExportGDateTime;
                    dtc.GIntervalTime = m_TaskData.GIntervalTime;

                    dtc.IsCustomHeader = m_TaskData.IsCustomHeader;
                    dtc.IsPublishHeader = m_TaskData.IsPublishHeader;
                    dtc.Headers = m_TaskData.Headers;

                    dtc.TaskRunType = m_TaskData.RunType;
                    if (m_TaskData.RunType == cGlobalParas.TaskRunType.OnlySave)
                    {
                        dtc.PublishType = m_TaskData.PublishType;
                        dtc.DataSource = m_TaskData.DataSource ;
                        dtc.TableName = m_TaskData.TableName;
                        dtc.InsertSql =m_TaskData.InsertSql;
