...
namespace NetMiner.Net.Socket
{
    public class HttpWebResponse
    {
        ...
        public string CharacterSet
        {
            get 
            {
                if (m_wCode == cGlobalParas.WebCode.auto)
                    return "";
                else
        ...
        public void Connect(HttpWebRequest request)
        {
            ...
            if (request.proxyType == cGlobalParas.ProxyType.HttpProxy)
            {
                remoteEP = new IPEndPoint(GetDomainIP(request.ProxyAddress.Trim()), request.ProxyPort);
            }
            else if (request.proxyType == cGlobalParas.ProxyType.None)
            {
                remoteEP = new IPEndPoint(GetDomainIP(ResponseUri.Host.Trim()), ResponseUri.Port);
            }
            else if (request.proxyType == cGlobalParas.ProxyType.SystemProxy)
            {
            ...
            else if (request.proxyType == cGlobalParas.ProxyType.Socket5)
            {
                //链接Socket5服务器
                remoteEP = new IPEndPoint(GetDomainIP(request.ProxyAddress.Trim()), request.ProxyPort);
            }
        ...
        public void SendRequestContinue(HttpWebRequest request)
        {
            ...
            if (request.method == cGlobalParas.RequestMethod.Post && !request.Headers.ContainsKey("Content-Type"))
            {
                request.Headers.Add("Content-Type", "application/x-www-form-urlencoded");
            }

            if (request.method == cGlobalParas.RequestMethod.Post && !request.Headers.ContainsKey("Content-Length"))
            {
            ...
            if (request.method == cGlobalParas.RequestMethod.Post)
            {
