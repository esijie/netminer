...
namespace NetMiner.Engine
{
    class cXmlIO
    {
        ...
        /// <summary>
        /// 在指定的节点下，插入一个完整的XML字符串
        /// </summary>
        public void InsertNodeValue(string NodeCollection, string Node, string condition, string ValueName,string NodeName,string xNode)
        {
            XmlNodeList fathernode = objXmlDoc.GetElementsByTagName(NodeCollection);
            XmlNodeList nodes = fathernode[0].ChildNodes;
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                for (int j = 0; j < nodes[i].ChildNodes.Count; j++)
                {
                    ...
                    if (nodes[i].ChildNodes[j].Name == Node && nodes[i].ChildNodes[j].InnerText == condition)
                    {
                        ...
                        objElement.InnerXml = xNode;
                        nod.AppendChild(objElement);
                        return;
                    }
                }
