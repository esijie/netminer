...
namespace SoukeyDataPublish.CustomControl
{
    public partial class uconMyDataOp : UserControl
    {
        ...
        private void IniTree()
        {
            ...
            //加载任务发布模板信息
            oPublishTask op = new oPublishTask(Program.getPrjPath());
            IEnumerable<ePublishTask> eps= op.LoadPublishData();
        ...
        private void StartPublish()
        {
            ...
            if (this.IsPublishByTemplate.Checked == true)
            {
                ...
                p.TemplateDBConn = this.txtTDbConn.Text;
            ...
            else
            {
                if (this.raPublishData.Checked == true)
                {
                    ...
                    p.DataSource = this.txtDataSource.Text;
            ...
            m_PControl = new cPublishControl(Program.getPrjPath());
            ...
            //定义一个cpublish
            cPublish pt = new cPublish(Program.getPrjPath(), m_PControl.PublishManage, p, this.IsErrLog.Checked, m_tmp);
        ...
        private void FillPublishInfo(string pName)
        {
            ...
            if (ep.PublishType== cGlobalParas.PublishType.publishTemplate)
            {
                ...
                for (int i = 0; i < paras.Count; i++)
                {
                    ...
                    this.dataParas.Rows[this.dataParas.Rows.Count - 1].Cells[0].Value = paras[i].DataLabel;
                    cGlobalParas.PublishParaType pType = paras[i].DataType;
                    ...
                    {
                        this.dataParas.Rows[this.dataParas.Rows.Count - 1].Cells[1].Value = paras[i].DataValue;
                        this.dataParas.Rows[this.dataParas.Rows.Count - 1].Cells[2].Value = "";
                    }
                    else if (pType == cGlobalParas.PublishParaType.Custom)
                    {
                        this.dataParas.Rows[this.dataParas.Rows.Count - 1].Cells[1].Value = "{手工输入}";
                        this.dataParas.Rows[this.dataParas.Rows.Count - 1].Cells[2].Value = paras[i].DataValue;
                    }
                }
            }
            else
        ...
        private void rmenuImport_Click(object sender, EventArgs e)
        {
            ...
            if (this.openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    NetMiner.Common.Tool.XMLUtil xUtil = new NetMiner.Common.Tool.XMLUtil();
                    ePublishTask ep =(ePublishTask) NetMiner.Common.Tool.XMLUtil.Deserialize(typeof(ePublishTask), sXML);
                    xUtil = null;

                    string pName =ep.pName;
                    if (p.IsExist(pName))
                    {
